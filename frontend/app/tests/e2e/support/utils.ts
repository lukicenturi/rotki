 ...
 * complete
 */
export function waitForAsyncQuery({ url, method }: QueryTarget, timeout = 120000) {
...
function createResult<T>(t: T): ActionResult<T> {
  return {
    message: '',
    result: t,
  };
...
function mockAsyncTaskBody(): ActionResult<{ task_id: number }> {
  return createResult({
    task_id: mockId++,
  });
}

export function mockRequest(target: QueryTarget, data: any = {}) {
  cy.intercept(target, {
    statusCode: 200,
