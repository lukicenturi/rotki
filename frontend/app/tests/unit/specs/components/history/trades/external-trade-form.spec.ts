...
describe('externalTradeForm.vue', () => {
  ...
  const createWrapper = (options: ComponentMountingOptions<typeof ExternalTradeForm> = {}) => mount(ExternalTradeForm, {
    global: {
      plugins: [
        pinia,
      ],
  ...
  describe('should prefill the fields based on the props', () => {
    it('no `editableItem` passed', async () => {
      ...
      expect(wrapper.find('[data-cy=type] [data-cy=trade-input-sell] input').attributes()).not.toHaveProperty('checked');
    ...
    it('`editableItem` passed', async () => {
      ...
      expect(
        (wrapper.find('[data-cy=trade-rate] [data-cy=primary] input').element as HTMLInputElement).value,
