...
describe('composables/use-chip-grouping', () => {
  ...
  describe('getGroupedOverflowCount', () => {
    it('returns count minus 1 for grouped keys', () => {
      ...
      expect(getGroupedOverflowCount('type')).toBe(4);
    });
    ...
    it('returns 0 for non-existent keys', () => {
      ...
      expect(getGroupedOverflowCount('nonexistent')).toBe(-1);
    });
  });
  ...
  describe('getGroupedItemsForKey', () => {
    it('returns all items for a specific key', () => {
      ...
      expect(getGroupedItemsForKey('type')).toEqual([item1, item2]);
    });
    ...
    it('returns empty array for non-existent key', () => {
      ...
      expect(getGroupedItemsForKey('nonexistent')).toEqual([]);
    });
  });
});
