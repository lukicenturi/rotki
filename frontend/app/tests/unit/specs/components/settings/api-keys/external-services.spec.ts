...
describe('/settings/api-keys/external-services', () => {
  ...
  describe('first time', () => {
    ...
    it('save the values when etherscan save is pressed', async () => {
      ...
      await nextTick();
      await wrapper
        .find('[data-cy=external-keys] [data-cy=etherscan] [data-cy=service-key__save]')
        .trigger('click');
      await flushPromises();
    ...
    it('save fails with an error', async () => {
      ...
      await wrapper.find('[data-cy=external-keys] [data-cy=etherscan] [data-cy=service-key__api-key] input')
        .setValue('123');
      await nextTick();
      await wrapper.find('[data-cy=external-keys] [data-cy=etherscan] [data-cy=service-key__save]')
        .trigger('click');
      await flushPromises();
  ...
  describe('the api returns value', () => {
    ...
    it('confirm and delete etherscan key', async () => {
      ...
      await wrapper
        .find('[data-cy=external-keys] [data-cy=etherscan] [data-cy=service-key__delete]')
        .trigger('click');
      await nextTick();
