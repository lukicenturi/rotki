...
// Matcher utilities composable (filteredMatchers not used here, we use actualFilteredMatchers below)
const {
  matcherForKey,
  matcherForKeyValue,
  validKeys,
} = useFilterMatchers(matchers, ref([]), search);
...
// Update filteredMatchers to use the actual selection
const {
  filteredMatchers: actualFilteredMatchers,
} = useFilterMatchers(matchers, selection, search);
...
async function applySuggestion(): Promise<void> {
  ...
  if (!get(selectedMatcher)) {
    const filteredMatchersVal = get(actualFilteredMatchers);
...
function moveSuggestion(up: boolean): void {
  const total = get(selectedMatcher) ? get(suggestedFilter).total : get(actualFilteredMatchers).length;
...
// Wrapper functions to handle slot item typing
function getItemSuggestion(item: unknown): Suggestion {
  return item as Suggestion;
}

function getItemDisplayType(item: unknown): 'normal' | 'grouped' | 'hidden' {
  return getChipDisplayType(getItemSuggestion(item));
}

function getItemEditMode(item: unknown): boolean {
  return isSuggestionBeingEdited(getItemSuggestion(item));
}

function getItemOverflowCount(item: unknown): number {
  return getGroupedOverflowCount(getItemSuggestion(item).key);
}

function getItemGroupedItems(item: unknown): Suggestion[] {
  return getGroupedItemsForKey(getItemSuggestion(item).key);
}
      ...
      >
        ...
        >
          <template #selection="{ item, chipAttrs }">
            <SelectionChip
              :item="getItemSuggestion(item)"
              :chip-attrs="chipAttrs"
              :display-type="getItemDisplayType(item)"
              :edit-mode="getItemEditMode(item)"
              :expanded-group-key="expandedGroupKey"
              :overflow-count="getItemOverflowCount(item)"
              :grouped-items="getItemGroupedItems(item)"
          ...
          <template #no-data>
            <FilterDropdown
              :matches="matches"
              :matchers="actualFilteredMatchers"
              :keyword="search"
