...
const enableForAllChain: Ref<boolean> = ref(false);
...
const openForm = (item: AddressBookEntry | null = null) => {
  ...
  if (item) {
    ...
    set(enableForAllChain, !item.blockchain);
  } else {
...
const hideForm = function () {
  set(showForm, false);
  set(form, emptyForm());
  set(enableForAllChain, false);
};

const valid = ref<boolean>(false);
const showForm = ref(false);
const editMode = ref<boolean>(false);
const form = ref<AddressBookPayload>(emptyForm());
...
const save = async () => {
  ...
  try {
    ...
    const payload = {
      ...
      name: formVal.name,
      blockchain: get(enableForAllChain) ? null : formVal.blockchain
    };
  ...
  <v-container>
      ...
      <template #search>
        <v-row class="pt-2 pb-6">
          <v-col cols="12" sm="4" md="3">
            <chain-select v-model="selectedChain" dense hide-details />
          </v-col>
    ...
    >
      <address-book-form
        v-model="form"
        :edit="editMode"
        :enable-for-all-chain="enableForAllChain"
        @valid="valid = $event"
        @update:enable-for-all-chain="enableForAllChain = $event"
      />
    </big-dialog>
  </v-container>
</template>
