...
def test_query_and_save_internal_transactions_returns_only_new_hashes(
    ...
    address = make_evm_address()
    existing_parent_tx = make_ethereum_transaction()
    new_parent_tx = make_ethereum_transaction()
    dbevmtx = DBEvmTx(database)
    with database.user_write() as write_cursor:
        dbevmtx.add_transactions(
            write_cursor=write_cursor,
    ...
    internal_existing = EvmInternalTransaction(
    ...
    internal_new = EvmInternalTransaction(
    ...
    with patch.object(
            ethereum_manager.node_inquirer,
            'get_transactions',
            return_value=iter([[internal_existing, internal_new]]),
    ), patch.object(
            ethereum_manager.node_inquirer,
