...
class Binance(ExchangeInterface, ExchangeWithExtras, SignatureGeneratorMixin):
    ...
    def _deserialize_asset_movement(self, raw_data: dict[str, Any]) -> list[AssetMovement]:
        ...
        extra_data = maybe_set_transaction_extra_data(
            address=deserialize_asset_movement_address(raw_data, 'address', asset := asset_from_binance(raw_data['coin'])),
            transaction_id=tx_id,
        )
        if (network := raw_data.get('network')) and (blockchain := BINANCE_NETWORK_TO_BLOCKCHAIN.get(network)):
