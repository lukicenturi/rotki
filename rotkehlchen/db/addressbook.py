    ...
    def delete_addressbook_entries(
        ...
        with self.write_ctx(book_type) as write_cursor:
            for address, blockchain in chain_addresses:
                ...
                if write_cursor.rowcount == 0:
                    raise InputError(
                        f'Addressbook entry with address "{address}" and blockchain {blockchain.value if blockchain is not None else None} doesnt '  # noqa: E501
